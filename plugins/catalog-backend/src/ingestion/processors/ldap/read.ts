...
export async function readLdapUsers(
  ...
  users: UserEntity[];
  userMemberOf: Map<string, Set<string>>; // DN to DN or UUID
}> {
...
export async function readLdapGroups(
  ...
  groups: GroupEntity[];
  groupMemberOf: Map<string, Set<string>>; // DN to DN or UUID, both group/user
  groupMember: Map<string, Set<string>>; // DN to DN or UUID, both group/user
}> {
  const { dn, options, set, map } = config;
  const entries = await client.search(dn, options);
...
export async function readLdapOrg(
  ...
  // the supported forms.
  const userMap: Map<string, UserEntity> = new Map();
  const groupMap: Map<string, GroupEntity> = new Map();
  ...
  // Fill in all of the immediate relations, keyed on metadata.name
  ...
  // Resolve and store in the intermediaries
  ...
  buildOrgHierarchy(groups, users, new Map());
